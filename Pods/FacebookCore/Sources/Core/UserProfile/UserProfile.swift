...
public struct UserProfile {
  ...
  /// A URL to the user's profile photo.
...
extension UserProfile {
  ...
  public static func fetch(userId: String, completion: @escaping Completion) {
    let request = GraphRequest(graphPath: userId,
                               parameters: ["fields" : "first_name,middle_name,last_name,name,email,link, picture.type(large)" as AnyObject],
                               httpMethod: .GET)
      ...
      case .success(let response):
        ...
        if let picture = responseDictionary?["picture"] as? [String : Any]{
            if let data = picture["data"] as? [String : Any] {
                photoUrlString = data["url"] as! String
            }
        }
